...
function github(version, type = "npm") {
  ...
  if (type === "github") {
    ...
    url = `https://registry.npmmirror.com/yapi-vendor/download/yapi-vendor-${version}.tgz`;
  } else {
    version = "v" + version;
...
module.exports = {
  ...
  getVersions: async function () {
    let info = await axios.get("https://registry.npmmirror.com/yapi-vendor");
    let versions = Object.keys(info.data.versions).filter(
