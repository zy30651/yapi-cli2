...
function connect(config) {
  ...
  return mongoose.connect(
    `mongodb://${config.db.servername}:${config.db.port}/${config.db.DATABASE}`,
    options
  );
}
...
function handleNpmInstall() {
  return new Promise(function (resolve, reject) {
    let child = shell.exec(
      "npm install -q --production --registry https://registry.npmmirror.com",
...
function handleServerInstall() {
  return new Promise(function (resolve, reject) {
    let child = shell.exec(`npm run install-server`, {
      async: true,
      silent: true,
    });
    child.stdout.on("data", (data) => {
...
async function run(argv) {
  ...
  try {
    ...
    utils.log(
      `部署成功，请切换到部署目录，输入： "node vendors/server/app.js" 指令启动服务器, 然后在浏览器打开 ${domain} 访问`
    );
  } catch (e) {
